#include "SourceModeGenerationD.h"
...
//设置圆波导的参数
void calculation::SourceModeGenerationD::SetSource_Circular(int sourcekind, int sourcetype,
...
void calculation::SourceModeGenerationD::SetSource_Rectangle(int sourcekind, int sourcetype, 
...
void calculation::SourceModeGenerationD::SetOutputProperty(int n1)
{
	N = n1;//采样点
...
bool calculation::SourceModeGenerationD::SetWaveguideMode(int m0, int n0)
{
...
bool calculation::SourceModeGenerationD::FieldCalculation_CircularT()
{
	//开始计算场分布
...
//计算并输出圆波导模式场分布 - 不改了
bool calculation::SourceModeGenerationD::FieldCalculation_Circular()
{
	//开始计算场分布
	double Lamda = C_Speed / Frequency;
	double K0 = 2 * Pi / Lamda;
...
bool calculation::SourceModeGenerationD::GetJ_R(vector<complex<double>> &_HPhi, vector<complex<double>> &_Hz, int _Nphi) {
	int Nphi = _Nphi;	double dp = 2 * Pi / Nphi; 
	SourceModeGenerationD::HPhi_r.resize(Nphi);
	SourceModeGenerationD::Hz_r.resize(Nphi);
	...
	for (int p = 0; p < Nphi; p++) {
		...
		if (Rotation != 1)//非左旋
		{
			SourceModeGenerationD::HPhi_r[p] = H_phi(Kz, Kr, SourceModeGenerationD::m, SourceModeGenerationD::Radius, phi);
			SourceModeGenerationD::Hz_r[p] = H_z(Kr, SourceModeGenerationD::m, SourceModeGenerationD::Radius, phi);
		}
		else //如果是左旋
		{
			SourceModeGenerationD::HPhi_r[p] = H_phi_LeftRo(Kz, Kr, SourceModeGenerationD::m, SourceModeGenerationD::Radius, phi);
			SourceModeGenerationD::Hz_r[p] = H_z_LeftRo(Kr, SourceModeGenerationD::m, SourceModeGenerationD::Radius, phi);
		}
	}

	for (int p = 0; p < Nphi; p++) {
		_HPhi[p] = SourceModeGenerationD::HPhi_r[p];
		_Hz[p] = SourceModeGenerationD::Hz_r[p];
	}

	SourceModeGenerationD::HPhi_r.resize(0);
	SourceModeGenerationD::Hz_r.resize(0);
}

//计算矩形波导场分布
bool calculation::SourceModeGenerationD::FieldCalculation_Rectangle()
{
	double Lamda = C_Speed / Frequency;
	double K0 = 2 * Pi / Lamda;
	double Omega = 2 * Pi * Frequency;
...
//设置场强归一化幅度值
bool calculation::SourceModeGenerationD::SetFieldAmplitude(double K)
{
...
bool calculation::SourceModeGenerationD::GetCircularWaveguideProperty(double &phi1, double &theta1, double &Rc, double &Lc)
{
...
bool calculation::SourceModeGenerationD::GetCircularConverterLauncherTriangle_TE0n(
...
bool calculation::SourceModeGenerationD::GetCircularSystemOpticalPath_TE0n_2Mirrors(
...
bool calculation::SourceModeGenerationD::GetCircularSystemOpticalPath_TE0n_3Mirrors(
	Vector3 & P4, Vector3 & V1, double & Length1, double & theta1,
...
bool calculation::SourceModeGenerationD::GetCircularSystemOpticalPath_TE0n_4Mirrors(
	Vector3 & P4, Vector3 & V1, double & Length1, double & theta1,
	                 Vector3 & V2, double & Length2, double & theta2,
...
void calculation::SourceModeGenerationD::GetEX(vector<vector<complex<double>>>& EX0)
{
	EX0.resize(N);
	for (int i = 0; i < N; i++)
	{
...
void calculation::SourceModeGenerationD::GetHX(vector<vector<complex<double>>>& HX0)
{
	HX0.resize(N);
	for (int i = 0; i < N; i++)
	{
...
void calculation::SourceModeGenerationD::GetEY(vector<vector<complex<double>>>& EY0)
{
	EY0.resize(N);
	for (int i = 0; i < N; i++)
	{
...
void calculation::SourceModeGenerationD::GetHY(vector<vector<complex<double>>>& HY0)
{
	HY0.resize(N);
	for (int i = 0; i < N; i++)
	{
...
void calculation::SourceModeGenerationD::GetEZ(vector<vector<complex<double>>>& EZ0)
{
	EZ0.resize(N);
	for (int i = 0; i < N; i++)
	{
...
void calculation::SourceModeGenerationD::GetHZ(vector<vector<complex<double>>>& HZ0)
{
	HZ0.resize(N);
	for (int i = 0; i < N; i++)
	{
